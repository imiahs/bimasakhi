...
const HomePage = () => {
    ...
    // Config
    const MODE = import.meta.env.VITE_DYNAMIC_HOME_MODE || 'off'; // off | preview | on
    const ROLLOUT_PCT = Number(import.meta.env.VITE_DYNAMIC_ROLLOUT_PERCENTAGE) || 0.1;
    ...
    const [isDynamicActive] = useState(() => {
        ...
        if (failCount >= FAILURE_LIMIT) {
            if (import.meta.env.DEV) logger.warn('HomePage', 'Dynamic Engine Disabled (Circuit Breaker)');
            return false;
        }
        ...
        if (params.get('previewDynamic') === 'true') {
            if (import.meta.env.DEV) logger.info('HomePage', 'Preview Mode Activated');
            return true;
        }
        ...
        const loadSections = async () => {
            ...
            try {
                ...
                if (isMounted.current) {
                    ...
                    // Empty data → fallback to static (no state change needed)
                }
            } catch (err) {
                logger.error('HomePage', 'Critical Dynamic Engine Error', err);

                // SESSION FAILURE COUNTER
    ...
    // Covers: Flag Off, Loading, Error, Empty Data, Network Fail, Timeout, Circuit Breaker
    return (
